...
import { garbageCollect } from './commands/maintenance.js';
...
function mutationOptions(cmd:Command):MutationOptions{const o=cmd.optsWithGlobals();return{dryRun:!!o.dryRun,yes:!!o.yes,project:o.project?resolve(o.project):undefined,agents:o.agent,mode:o.mode};}
...
export function buildProgram():Command{
  ...
  install.action(async(ids,_,cmd)=>{const ctx=optionContext(cmd),opts={...mutationOptions(cmd),collection:cmd.opts().collection};await mutate(cmd,()=>installSkills(ctx,ids,{...opts,dryRun:true}),()=>installSkills(ctx,ids,opts));});
  for(const enable of[true,false])targeting(writing(program.command((enable?'enable':'disable')+' <ids...>').description(enable?'把已装技能接入Agent':'停用指定Agent投影，保留完整内容'))).action(async(ids,_,cmd)=>{const ctx=optionContext(cmd),opts=mutationOptions(cmd);await mutate(cmd,()=>toggleSkills(ctx,ids,enable,{...opts,dryRun:true}),()=>toggleSkills(ctx,ids,enable,opts));});
  targeting(writing(program.command('remove <ids...>').alias('uninstall').description('卸载当前范围选择，其他引用不受影响'))).action(async(ids,_,cmd)=>{const ctx=optionContext(cmd),opts=mutationOptions(cmd);await mutate(cmd,()=>removeSkills(ctx,ids,{...opts,dryRun:true}),()=>removeSkills(ctx,ids,opts));});
  ...
  program.command('files <id>').description('完整文件清单与摘要').option('--project <directory>','项目范围').action(async(id,_,cmd)=>{const state=await loadState(optionContext(cmd));emit(cmd,getRelease(state,id,cmd.opts().project).manifest.files);});
  program.command('status').description('本机状态，不联网').action(async(_,cmd)=>emit(cmd,await localStatus(optionContext(cmd))));
  program.command('check').description('只读比较版本，不更新文件或目录缓存').option('--project <directory>','项目范围').action(async(_,cmd)=>emit(cmd,await checkUpdates(optionContext(cmd),mutationOptions(cmd))));
  writing(program.command('update [ids...]').description('预览后更新未固定的已安装技能')).action(async(ids,_,cmd)=>{const ctx=optionContext(cmd),opts=mutationOptions(cmd);await mutate(cmd,()=>updateSkills(ctx,ids,{...opts,dryRun:true}),()=>updateSkills(ctx,ids,opts));});
  for(const pinned of[true,false])writing(program.command((pinned?'pin':'unpin')+' <ids...>').description(pinned?'固定当前技能版本':'允许后续显式更新')).action(async(ids,_,cmd)=>{const ctx=optionContext(cmd),opts=mutationOptions(cmd);await mutate(cmd,()=>pinSkills(ctx,ids,pinned,{...opts,dryRun:true}),()=>pinSkills(ctx,ids,pinned,opts));});
  writing(program.command('rollback <id>').description('切换回保留的旧版本')).option('--version <version>','保留的版本，默认上一个').action(async(id,_,cmd)=>{const ctx=optionContext(cmd),opts=mutationOptions(cmd);await mutate(cmd,()=>rollbackSkill(ctx,id,cmd.opts().version,{...opts,dryRun:true}),()=>rollbackSkill(ctx,id,cmd.opts().version,opts));});
  writing(program.command('sync').description('按项目锁定版本恢复，不升级')).requiredOption('--frozen','严格恢复锁文件').action(async(_,cmd)=>{const ctx=optionContext(cmd),opts=mutationOptions(cmd);await mutate(cmd,()=>syncFrozen(ctx,{...opts,dryRun:true}),()=>syncFrozen(ctx,opts));});
  ...
  writing(agents.command('remove <target>').description('移除目标及自身引用，不影响其他Agent'),false).action(async(id,_,cmd)=>{const ctx=optionContext(cmd),opts=mutationOptions(cmd);await mutate(cmd,()=>removeAgent(ctx,id,{...opts,dryRun:true}),()=>removeAgent(ctx,id,opts));});
  ...
  writing(program.command('import <path>').description('导入前核验内容，不复制其他电脑的目录配置')).action(async(path,_,cmd)=>{const ctx=optionContext(cmd),opts=mutationOptions(cmd);await mutate(cmd,()=>importLibrary(ctx,path,{...opts,dryRun:true}),()=>importLibrary(ctx,path,opts));});
  writing(program.command('fork <id>').description('生成独立可编辑副本')).requiredOption('--output <path>','必须是新目录').action(async(id,_,cmd)=>{const ctx=optionContext(cmd),opts=mutationOptions(cmd);await mutate(cmd,async()=>({id,output:resolve(cmd.opts().output),managed:false}),()=>forkSkill(ctx,id,cmd.opts().output,opts.project));});
  ...
  writing(program.command('gc').description('预览并清理无引用孤立对象，保留历史'),false).action(async(_,cmd)=>{const ctx=optionContext(cmd),opts=mutationOptions(cmd);await mutate(cmd,()=>garbageCollect(ctx,{dryRun:true}),()=>garbageCollect(ctx,{...opts,yes:true}));});
  ...
  program.command('run <id> [arguments...]').description('显式执行已审核本地工具，可能调用服务商计费').option('--project <directory>','项目范围/媒体输入相对根').allowUnknownOption(true).passThroughOptions().action(async(id,args,_,cmd)=>{const ctx=optionContext(cmd),r=getRelease(await loadState(ctx),id,cmd.opts().project);const forwarded=args[0]==='--'?args.slice(1):args;const result=await runSkill(ctx,r,forwarded,{project:cmd.opts().project,capture:ctx.json});emit(cmd,result);if(typeof result==='object'&&result&&'exitCode'in result)process.exitCode=Number(result.exitCode)||0;});
  const migrate=program.command('migrate').description('旧系统只读迁移');
  writing(migrate.command('panel').description('只读导入旧Python管理的内容，不读Key')).requiredOption('--from <directory>','旧客户端home').action(async(_,cmd)=>{const ctx=optionContext(cmd),opts=mutationOptions(cmd);await mutate(cmd,()=>migratePanel(ctx,cmd.opts().from,{...opts,dryRun:true}),()=>migratePanel(ctx,cmd.opts().from,opts));});
