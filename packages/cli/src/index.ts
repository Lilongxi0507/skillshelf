...
import { addAgent, checkUpdates, doctor, forkSkill, installSkills, listAgents, listSkills, localStatus, migrateSources, pinSkills, readSkill, removeAgent, removeSkills, rollbackSkill, skillHistory, syncFrozen, toggleSkills, updateSkills, verifyInstalled } from './manager.js';
import { serveDiscovery } from './mcp/discovery.js';
...
export function buildProgram():Command{
  ...
  program.command('history <id>').description('列出保留的安装历史与来源身份').option('--project <directory>','项目范围').action(async(id,_,cmd)=>emit(cmd,await skillHistory(optionContext(cmd),id,cmd.opts().project)));
  ...
  writing(program.command('rollback <id>').description('切换回保留的旧版本')).option('--version <version>','保留的 npm SemVer，默认上一个').option('--revision <key>','保留的精确 release 键（schema-3 GitHub 历史用）').action(async(id,_,cmd)=>{const ctx=optionContext(cmd),opts=await mutationOptions(cmd);await mutate(cmd,()=>rollbackSkill(ctx,id,cmd.opts().version,{...opts,revision:cmd.opts().revision,dryRun:true}),()=>rollbackSkill(ctx,id,cmd.opts().version,{...opts,revision:cmd.opts().revision}));});
  ...
  mcp.command('serve').description('只读 MCP stdio 服务：搜索/查看/读取/诊断，不安装不执行不写入').action(async(_,cmd)=>{const ctx=optionContext(cmd);if(!cmd.optsWithGlobals().json&&process.stdin.isTTY)fail('USAGE','mcp serve 面向 stdio 客户端；交互终端请使用其他子命令');await serveDiscovery({...ctx,offline:true});});
  const sources=program.command('sources').description('固定 GitHub 来源只读报告');
  sources.command('check').description('列出目录固定来源身份；不联网、不安装、不发布').action(async(_,cmd)=>{const catalog=await loadCatalog(optionContext(cmd));const tuples=new Map<string,{repository:string;commit:string;members:string[]}>();for(const entry of catalog.skills){if(entry.sourceManifest?.acquisition.kind!=='github')continue;const key=`${entry.sourceManifest.acquisition.repository}@${entry.sourceManifest.acquisition.commit}`;const tuple=tuples.get(key)??{repository:entry.sourceManifest.acquisition.repository,commit:entry.sourceManifest.acquisition.commit,members:[]};tuple.members.push(entry.id);tuples.set(key,tuple);}emit(cmd,{wrote:false,network:'not-requested',sources:[...tuples.values()].map(tuple=>({repository:tuple.repository,commit:tuple.commit,members:tuple.members.sort(),status:'unknown',checked:false})).sort((a,b)=>a.repository<b.repository?-1:1)});});
  ...
  writing(migrate.command('sources').description('把 npm 安装迁移到固定 GitHub 来源，保留历史')).option('--skill <ids...>','仅迁移指定技能').action(async(_,cmd)=>{const ctx=optionContext(cmd),opts=await mutationOptions(cmd);await mutate(cmd,()=>migrateSources(ctx,{...opts,ids:cmd.opts().skill,dryRun:true}),()=>migrateSources(ctx,{...opts,ids:cmd.opts().skill}));});
