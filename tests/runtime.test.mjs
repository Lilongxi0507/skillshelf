...
test('run uses verified artifact and minimal private config, safe argv/env, isolated cwd/outputs and cleanup', posix, async (t) => {
  ...
  assert.deepEqual(record.argv.slice(0, 4), ['-B', '-E', '-s', '-S']);
  assert.deepEqual(record.argv.slice(5), args);
  ...
  for (const key of ['PANEL_API_KEY', 'NODE_OPTIONS', 'PYTHONPATH', 'SKILLSHELF_TEST_SEARCH_KEY']) assert.equal(record.environment.includes(key), false);
